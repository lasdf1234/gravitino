...
| `gravitino.catalog.classloader.sharing.enabled`        | Whether to share one ClassLoader across catalogs that have identical isolation-relevant properties. When `true` (default), such catalogs reuse a single ClassLoader, reducing Metaspace usage; when `false`, each catalog gets its own ClassLoader as in releases before 2.0.0.                                                                                                                          | `true`    | No | 2.0.0 |
